...
import { test } from '../actions/BuyRequestActions';
...
initMaturityDate.setDate(initMaturityDate.getDate() + MAX_UI_OPTION_DURATION);
...
export default class MintOption extends React.Component {
    ...
    async setUnderlyingTokenId(tokenId) {
        ...
        if (underlyingToken) {
            ...
            this.setState({ underlyingTokenId: filteredTokenId, strikePrice: oraclePrice, oraclePrice: oraclePrice, oraclePriceGraph: oraclePrice });
        } else {
            this.setState({ underlyingTokenId: filteredTokenId, oraclePrice: undefined, oraclePriceGraph: undefined });
        }
    };
    ...
    render() {
        ...
        const txFees = TX_FEES.map(i => { return { value: i, label: formatERGAmount(i) + " ERG" } });
        ...
        //const underlyingTokensAvailableAmount = currentToken.amount / Math.pow(10,currentToken.decimals) ?? 0;
        console.log("render underlyingTokens", underlyingTokens)
        return (
            <Fragment >
                <div className="card zonemint p-1 m-2">
                    <div className='card zonemint d-flex flex-column m-2 p-2'>
                            ...
                            <div>
                                <DatePicker
                                    ...
                                    excludeDateIntervals={[
                                        { start: 0, end: now },
                                        { start: new Date((new Date()).setMonth((new Date()).getMonth() + MAX_UI_OPTION_DURATION)), end: new Date("2999-12-31") }
                                    ]}
                                />
                            </div>
                        </div>
                        ...
                        <div className='d-flex flex-row justify-content-between align-items-end'>
                            <label htmlFor="txFee" className='col-sm-2 d-flex align-items-start'>Transaction miner fee</label>
                            <div className='w-100 d-flex flex-row'>
                            ...
                            <strong>
                                Cost: &nbsp;
                                {
                                    this.state.optionType === 'Call' && currentToken ?
                                        <div>
                                            {formatERGAmount(4 * this.state.txFee + 2 * MIN_NANOERG_BOX_VALUE + mintFee)} ERG -
                                            {this.state.shareSize * this.state.optionAmount}
                                            {" " + currentToken.name}
                                        </div>
                                        :
                                        <div>
                                            {formatERGAmount(4 * this.state.txFee + 2 * MIN_NANOERG_BOX_VALUE + mintFee + this.state.shareSize * this.state.optionAmount * this.state.strikePrice)} ERG
                                        </div>
                                }
                            </strong>
                        </div>
                                    ...
                                    <div className='d-flex flex-column m-1 p-1 zonegraph'>
                                        ...
                                        <div className='graph-container'>
                                            <OptionPriceTimeChart
                                                ...
                                                oraclePrice={this.state.oraclePriceGraph * Math.pow(10, currentToken.decimals)}
                                                strikePrice={this.state.strikePrice * Math.pow(10, currentToken.decimals)}
                                    ...
                                    <div className='d-flex flex-column m-1 p-1 zonegraph'>
                                        ...
                                        <div className='graph-container'>
                                            <OptionPriceUnderlyingPriveChart
                                                ...
                                                optionStyle={this.state.optionStyle}
                                                maturityDate={this.state.maturityDate}
                                                oraclePrice={this.state.oraclePrice * Math.pow(10, currentToken.decimals)}
                                                strikePrice={this.state.strikePrice * Math.pow(10, currentToken.decimals)}
                                                shareSize={this.state.shareSize}
                                                sigma={this.state.sigma}
                                                K1={this.state.K1}
